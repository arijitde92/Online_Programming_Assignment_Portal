...
class CodeAnalyzer:
    ...
    def load_code(self, filepath, debug=False):
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """Please analyze the following C code and provide feedback within 200 words overall on the following points:
    ...
    def analyze_compilation_error(self, compile_errors, debug=False):
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """As a programming mentor, analyze the following C compilation errors in the context of the code 
    ...
    def analyze_runtime_error(self, runtime_errors, test_input=None, debug=False):
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """As a programming mentor, analyze the following C runtime errors in the context of the code 
                and its previous analysis. Provide helpful hints for fixing them. DO NOT provide the complete solution, 
                only give hints that will help the student learn and understand how to fix the errors themselves.
    ...
    def analyze_test_case_mismatch(self, test_input, actual_output, expected_output, debug=False):
        """
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """As a programming mentor, analyze the following test case mismatch in the context of the code 
...
def get_code_feedback(filepath, debug=False):
    ...
    if analyzer.load_code(filepath, debug):
        return {
            "status": "success",
            "feedback": analyzer.code_analysis
        }
    return {
        "status": "error",
        "feedback": analyzer.code_analysis
...
def get_test_case_feedback(test_input, actual_output, expected_output, debug=False):
    ...
    try:
        ...
        feedback = analyzer.analyze_test_case_mismatch(test_input, actual_output, expected_output, debug)
        return {
            "status": "success",
            "feedback": feedback
        }
